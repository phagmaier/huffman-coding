...
std::vector<std::pair<char,int>> get_freq(std::string &str){
  ...
  //in reality gonna have to sort it
   std::sort(sorted.begin(), sorted.end(), [](const std::pair<char, int>& a, const std::pair<char, int>& b) {
        return a.second < b.second;  // Compare based on the second value (frequency)
    });
